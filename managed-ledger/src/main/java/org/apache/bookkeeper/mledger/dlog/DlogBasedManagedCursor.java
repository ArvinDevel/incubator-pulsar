...
public class DlogBasedManagedCursor implements ManagedCursor {
    ...
    protected static final AtomicReferenceFieldUpdater<DlogBasedManagedCursor, DlogBasedOpReadEntry> WAITING_READ_OP_UPDATER =
            AtomicReferenceFieldUpdater.newUpdater(DlogBasedManagedCursor.class, DlogBasedOpReadEntry.class, "waitingReadOp");
    @SuppressWarnings("unused")
    private volatile DlogBasedOpReadEntry waitingReadOp = null;

    private static final int FALSE = 0;
    private static final int TRUE = 1;
    private static final AtomicIntegerFieldUpdater<DlogBasedManagedCursor> RESET_CURSOR_IN_PROGRESS_UPDATER = AtomicIntegerFieldUpdater
            .newUpdater(DlogBasedManagedCursor.class, "resetCursorInProgress");
    @SuppressWarnings("unused")
    private volatile int resetCursorInProgress = FALSE;
    private static final AtomicIntegerFieldUpdater<DlogBasedManagedCursor> PENDING_READ_OPS_UPDATER =
            AtomicIntegerFieldUpdater.newUpdater(DlogBasedManagedCursor.class, "pendingReadOps");
    @SuppressWarnings("unused")
    ...
    private static final AtomicIntegerFieldUpdater<DlogBasedManagedCursor> PENDING_MARK_DELETED_SUBMITTED_COUNT_UPDATER =
            AtomicIntegerFieldUpdater.newUpdater(DlogBasedManagedCursor.class, "pendingMarkDeletedSubmittedCount");
    @SuppressWarnings("unused")
    ...
    private static final AtomicReferenceFieldUpdater<DlogBasedManagedCursor, State> STATE_UPDATER =
            AtomicReferenceFieldUpdater.newUpdater(DlogBasedManagedCursor.class, State.class, "state");
        ...
        ledger.getStore().asyncGetCursorInfo(ledger.getName(), name, new MetaStoreCallback<ManagedCursorInfo>() {
            ...
            public void operationComplete(ManagedCursorInfo info, Stat stat) {
                ...
                if (info.getCursorsLedgerId() == -1L) {
                    ...
                    recoveredCursor(recoveredPosition, recoveredProperties);                    callback.operationComplete();
                } else {
...
//        bookkeeper.asyncOpenLedger(ledgerId, config.getDigestType(), config.getPassword(), (rc, lh, ctx) -> {
    ...
    void initialize(PositionImpl position, final VoidCallback callback) {
        ...
        createNewMetadataLedger(new VoidCallback() {
            ...
            public void operationComplete() {
                STATE_UPDATER.set(DlogBasedManagedCursor.this, State.Open);
                callback.operationComplete();
            }
    ...
    public long getNumberOfEntriesInBacklog() {
        if (log.isDebugEnabled()) {
            log.debug("[{}] Consumer {} cursor ml-entries: {} -- deleted-counter: {} other counters: mdPos {} rdPos {}",
                    ledger.getName(), name, DlogBasedManagedLedger.ENTRIES_ADDED_COUNTER_UPDATER.get(ledger), messagesConsumedCounter,
                    markDeletePosition, readPosition);
        }
        long backlog = DlogBasedManagedLedger.ENTRIES_ADDED_COUNTER_UPDATER.get(ledger) - messagesConsumedCounter;
        if (backlog < 0) {
    ...
    public void asyncFindNewestMatching(FindPositionConstraint constraint, Predicate<Entry> condition,
        ...
        op = new DlogBasedOpFindNewest(this, startPosition, condition, max, callback, ctx);
        op.find();
    }
        ...
        VoidCallback finalCallback = new VoidCallback() {
            ...
            public void operationComplete() {
                ...
                synchronized (pendingMarkDeleteOps) {
                    ...
                    if (!RESET_CURSOR_IN_PROGRESS_UPDATER.compareAndSet(DlogBasedManagedCursor.this, TRUE, FALSE)) {
    ...
    public void resetCursor(Position newPos) throws ManagedLedgerException, InterruptedException {
        ...
        if (!counter.await(DlogBasedManagedLedger.AsyncOperationTimeoutSeconds, TimeUnit.SECONDS)) {
            if (result.exception != null) {
    ...
    void internalMarkDelete(final MarkDeleteEntry mdEntry) {
        ...
        persistPosition(cursorLedger, mdEntry, new VoidCallback() {
            ...
            public void operationComplete() {
                ...
                ledger.updateCursor(DlogBasedManagedCursor.this, mdEntry.newPosition);
            ...
            public void operationFailed(ManagedLedgerException exception) {
                log.warn("[{}] Failed to mark delete position for cursor={} position={}", ledger.getName(),
                        DlogBasedManagedCursor.this, mdEntry.newPosition);
                if (log.isDebugEnabled()) {
    ...
    public void close() throws InterruptedException, ManagedLedgerException {
        ...
        if (!latch.await(DlogBasedManagedLedger.AsyncOperationTimeoutSeconds, TimeUnit.SECONDS)) {
...
//                asyncDeleteLedger(cursorLedger);
        ...
        createNewMetadataLedger(new VoidCallback() {
            ...
            public void operationFailed(ManagedLedgerException exception) {
                ...
                synchronized (pendingMarkDeleteOps) {
                    ...
                    STATE_UPDATER.set(DlogBasedManagedCursor.this, State.NoLedger);
                }
            }
        });
    }
    ...
    private List<MLDataFormats.LongProperty> buildPropertiesMap(Map<String, Long> properties) {
        ...
        List<MLDataFormats.LongProperty> longProperties = Lists.newArrayList();
        properties.forEach((name, value) -> {
            MLDataFormats.LongProperty lp = MLDataFormats.LongProperty.newBuilder().setName(name).setValue(value).build();
    ...
    void persistPosition(final LedgerHandle lh, MarkDeleteEntry mdEntry, final VoidCallback callback) {
        ...
        lh.asyncAddEntry(pi.toByteArray(), (rc, lh1, entryId, ctx) -> {
            if (rc == BKException.Code.OK) {
                ...
                STATE_UPDATER.compareAndSet(DlogBasedManagedCursor.this, State.Open, State.NoLedger);
                callback.operationFailed(new ManagedLedgerException(BKException.getMessage(rc)));
            }
        }, null);
    }
    ...
    void notifyEntriesAvailable() {
        ...
        DlogBasedOpReadEntry DlogBasedOpReadEntry = WAITING_READ_OP_UPDATER.getAndSet(this, null);

        if (DlogBasedOpReadEntry != null) {
            if (log.isDebugEnabled()) {
                log.debug("[{}] [{}] Received notification of new messages persisted, reading at {} -- last: {}",
                        ledger.getName(), name, DlogBasedOpReadEntry.readPosition, ledger.lastConfirmedEntry);
    ...
    private static final Logger log = LoggerFactory.getLogger(DlogBasedManagedCursor.class);
}
